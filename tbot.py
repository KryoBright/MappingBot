...
def handle_location(message):
	...
	q=User.addCoordinateMessage(message_lp,msg_id[0],message.message_id)
	print(q)
	User.CoordinateMessagePropUpdate(msg_id[0], "lat", message.location.latitude)
	User.CoordinateMessagePropUpdate(msg_id[0], "long", message.location.longitude)
	User.addUserMessageRelationship(message.from_user.id,msg_id[0], "from_User")
	msg_id[0]+=1
...
def update_loc(user_tg_id,meetpoint):
	...
	if (res==True)or(len(msg)==0):
		...
		q=User.addCoordinateMessage(85000+time.time(),msg_id[0], msg.message_id)
		User.CoordinateMessagePropUpdate(msg_id[0], "lat", loc[0])
		User.CoordinateMessagePropUpdate(msg_id[0], "long", loc[1])
		print(User.addUserMessageRelationship(user_tg_id,msg_id[0], "from_Bot"))
